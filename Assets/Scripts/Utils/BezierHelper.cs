...
public class BezierHelper  {
	...
	public static GameObject moveTo(GameObject fromGameObject, GameObject uiTarget, GameObject prefab, float time, float delay){

        Camera gameCamera = NGUITools.FindCameraForLayer(fromGameObject.layer);
       ...
       /* UILabel label = guiObject.GetComponentInChildren<UILabel>();
        ...
        guiObject.transform.position = fromGameObject.transform.position;
        ...
        guiObject.MoveTo(path, time, delay, EaseType.easeInSine, true);
        guiObject.RotateBy(new Vector3(0, 0, 1f), 0.5f, 0, EaseType.easeInOutSine, LoopType.loop, true);
